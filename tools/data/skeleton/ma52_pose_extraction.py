...
args.det_checkpoint = 'weights/faster_rcnn_r50_fpn_1x_coco-person_20201216_175929-d022e227.pth'  # noqa: E501
args.det_score_thr = 0.5
args.pose_config = 'demo/demo_configs/td-hm_hrnet-w32_8xb64-210e_coco-256x192_infer.py'  # noqa: E501
args.pose_checkpoint = 'weights/hrnet_w32_coco_256x192-c78dce93_20200708.pth'  # noqa: E501
...
def ntu_pose_extraction(vid, bbox, args):
    ...
    det_results=[bbox]*len(frame_paths)
    ...
    anno['label'] = int(osp.basename(vid).split('_')[1])
    tmp_dir.cleanup()
...
if __name__ == '__main__':
    ...
    with open('/home/wangchen/projects/datasets/Microaction-52/instances_bbox.pickle', 'rb') as fr:
        det_results = pickle.load(fr)
    # video_name = os.path.basename(args.video)
    # bbox = det_results[video_name]
    # anno = ntu_pose_extraction(args.video, np.array([bbox]), args)
    # path_save = os.path.join(args.output, video_name.replace(".mp4", ".pkl"))
    # mmengine.dump(anno, path_save)
    ...
    for path_video in tqdm.tqdm(video_list):
        ...
        bbox = det_results[video_name]
        anno = ntu_pose_extraction(path_video, np.array([bbox]), args)
        path_save = os.path.join(args.output, video_name.replace(".mp4", ".pkl"))
        mmengine.dump(anno, path_save)
