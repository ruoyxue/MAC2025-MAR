...
class VideoDataset(BaseActionDataset):
    ...
    def __init__(self,
        ...
        if test_mode:
            # with open('/data/xueruoyao/ActionAnalysis_dataset/MA-52/test_subset_instances/all_instance.pickle', 'rb') as fr:
            with open('/data/xueruoyao/ActionAnalysis_dataset/MA-52/val_instances/all_instance.pickle', 'rb') as fr:
                self.bboxes = pickle.load(fr)
        else:
            with open('/data/xueruoyao/ActionAnalysis_dataset/MA-52/train_val_instances/all_instance.pickle', 'rb') as fr:
                self.bboxes = pickle.load(fr)
        super().__init__(
            ann_file,
            pipeline=pipeline,
        ...
        for line in fin:
            ...
            data_list.append(
                dict(
                    filename=filename,
                    label=label,
                    bbox=self.bboxes.get(filename, None),
                )
            )
        return data_list
