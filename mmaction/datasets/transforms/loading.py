...
class DecordDecodeCrop(DecordDecode):
    ...
    def __init__(self, mode: str = 'accurate', train=True, scale=(224, 448)):
        super().__init__(mode)
        self.train = train
    ...
    def transform(self, results: Dict) -> Dict:
        ...
        if self.train:
            ...
            # r = np.clip(np.random.randn()*rf, -rf, rf) * (1 if random.random() <= 0.5 else -1)
            offset = 0.1
        ...
        # resize the gt_bboxes and proposals to their real scale
        if 'gt_bboxes' in results:
            h, w = results['img_shape']
            scale_factor = np.array([w, h, w, h])
            gt_bboxes = results['gt_bboxes']
