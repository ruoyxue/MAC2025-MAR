...
class VisionTransformerRes(BaseModule):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        # x = self.norm(x)
        # feature_sum = self.norm(feature_sum)
        final_feature = x + feature_sum
        ...
        if self.fc_norm is not None:
            # final_feature = feature_sum + x
            return self.fc_norm(final_feature.mean(1))
