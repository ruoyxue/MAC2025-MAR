...
class VisionTransformerDeep(BaseModule):
    ...
    def __init__(self,
        ...
        if use_mean_pooling:
            ...
            self.fc_norms = nn.ModuleList(
                [build_norm_layer(norm_cfg, embed_dims)[1] for _ in range(4)]
            )
        else:
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        x = self.norms[0](x)
        intermediate_features = [
            norm(feat) for norm, feat in zip(self.norms[1:], intermediate_features)
        ]
        ...
        if self.fc_norms is not None:
            return (
                self.fc_norms[0](x.mean(1)),
                [
                    fn(feat.mean(1))
                    for fn, feat in zip(self.fc_norms[1:], intermediate_features)
                ],
            )

        return (x[:, 0], [feat[:, 0] for feat in intermediate_features])
